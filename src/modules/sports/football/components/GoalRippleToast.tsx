...
const EMERALD = "var(--color-accent-sports)";
const GOLD = "var(--color-gold)";

// Football's copy of the Goals "+1" ripple — the felt moment when logging a match moves a Goal.
// Same structure as Watching's GoalRippleToast (count ticks up, bar fills old → new), emerald accent
// and a football icon.
export function GoalRippleToast({
  ...
  const accent = reached ? GOLD : EMERALD;
  ...
  return (
    ...
    >
      ...
      >
        {reached ? <Trophy size={17} /> : <Goal size={17} />}
      </div>
