...
export function useAddComment(taskId: string) {
  ...
  return useMutation({
    mutationFn: (text: string) =>
      TaskService.logActivity({ task_id: taskId, action: "commented", changes: { text } }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ACTIVITY_KEYS.byTask(taskId) });
    },
  });
...
export function useUpdateComment(taskId: string) {
  ...
  return useMutation({
    mutationFn: ({ activityId, text }: { activityId: string; text: string }) =>
      TaskService.updateComment(activityId, text),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ACTIVITY_KEYS.byTask(taskId) });
    },
  });
...
export function useDeleteComment(taskId: string) {
  ...
  return useMutation({
    mutationFn: (activityId: string) => TaskService.deleteComment(activityId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ACTIVITY_KEYS.byTask(taskId) });
    },
  });
