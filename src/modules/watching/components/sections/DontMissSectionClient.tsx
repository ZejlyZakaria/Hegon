...
import {
  useOwnedTmdbIds,
  useOwnedMediaIds,
} from "@/modules/watching/hooks/useOwnedTmdbIds";
    ...
    >
      ...
      {posterUrl && (
        <div className="absolute inset-0">
          ...
          <Image
            src={tmdbImage(posterUrl, "w92") || posterUrl}
            alt=""
            aria-hidden
            fill
            loading="lazy"
            sizes="96px"
            className="scale-[1.7] object-cover blur-3xl"
          />
      ...
      >
        ...
        {posterUrl && (
          <Image
            ...
            className={cn(
              "object-cover",
              !imgInstant && "transition-opacity duration-200",
            )}
      ...
      <div className="absolute inset-0">
        ...
        >
          ...
          <div className="absolute inset-x-0 bottom-0 flex flex-col gap-1 px-3 pb-3">
            <p className="truncate text-micro font-semibold text-white">
              {title}
            </p>
...
function TrendingMobileCard({
  ...
  return (
    ...
    >
      {posterUrl && (
        <Image
          src={tmdbImageFor(posterUrl, 170) || posterUrl}
          alt={title}
          fill
          loading="lazy"
...
export default function DontMissSectionClient({
  config,
}: {
  ...
  const { data: ownedIds = [] } = useOwnedTmdbIds(
    userId ?? "",
    config.type,
    !!userId,
  );
  ...
  const items = useMemo(() => {
    ...
    const recs = (data.recommendations as any[]).filter(
      (r) => r.id !== trendingId,
    );
