...
export function FloatingSaveBar({ isDirty, isPending, onSave, onDiscard }: Props) {
  return (
    <div
      className={cn(
        "fixed bottom-6 left-1/2 z-50 -translate-x-1/2 transition-[transform,opacity] duration-300",
        isDirty
          ? "pointer-events-auto translate-y-0 opacity-100"
          : "pointer-events-none translate-y-4 opacity-0",
      )}
    >
      <div className="flex items-center gap-3 rounded-2xl border border-border-default bg-surface-2/95 py-2.5 pl-5 pr-2.5 shadow-lg backdrop-blur-xl">
