...
// app/api/football/sync-competition/[externalId]/route.ts
//
// Bulk cache-fill for a WHOLE competition's season into sport.football_matches — called server-side
// when a competition is followed. One football-data call returns the full fixture list (~380 for a
// league). Same admin/upsert/batch pattern as sync-team; the client never sees the key.
// `externalId` is the competition's football-data id (football_competitions.api_external_id).
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ externalId: string }> }) {
  ...
  const res = await fetch(`${FD_BASE}/competitions/${externalId}/matches`, {
