...
if (latestStamp > through) {
  console.error(
    [
      ...
      "  2. relever les comptages réels — au choix :",
      "     a. Supabase → SQL Editor : coller supabase/checks/schema-drift.sql SANS les lignes",
      "        `\\pset` (commandes psql, pas du SQL) → Run → six lignes ;",
      "     b. GitHub → Actions → Watchdog → Run workflow → étape « Dérive du schéma »",
      "        (elle imprime les six comptages même quand elle est rouge) ;",
      "     c. psql \"$DB_URL\" -f supabase/checks/schema-drift.sql",
